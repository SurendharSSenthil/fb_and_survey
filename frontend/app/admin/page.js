...
import { Layout, Card, Form, Input, Button, Table, Select, Space, Typography, Alert, Statistic, Row, Col, List, Divider, Modal, InputNumber, message, Skeleton, Empty } from 'antd'
...
export default function AdminPage() {
  ...
  const handleCreateDepartment = async () => {
    if (!newDeptCode || !newDeptName) {
      message.error('Please enter department code and name')
      return
    }
  ...
  const handleLoadCourses = async () => {
    ...
    try {
      ...
      if (response.data.length > 0) {
        ...
        message.info('No courses found for the selected criteria')
      }
    } catch (err) {
  ...
  const headerContent = (
    <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <Space>
        <Button style={{ color: '#fff', margin: 0 }} icon={<HomeTwoTone />} onClick={() => router.push('/')} ghost>
          Home
        </Button>
      </Space>
    </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedDept && selectedYear && selectedSemester) {
                      ...
                      message.error('Please select a department, year and semester first')
                    }
                  }}
                >
                  Show Stats
      ...
      {(hasSearched || loading) && (
        <div style={{ marginTop: 24 }}>
          ...
          {!loading && courses.length === 0 ? (
            <Card>
              <Empty description="No courses found for the selected department, year, and semester." />
            </Card>
          ) : (
            <Row gutter={[16, 16]}>
              {loading ? (
